...
export function useDirectMessages({ dmRoomId }) {
  ...
  const addMessage = useCallback((message) => {
    setMessages(prev => {
      ...
      const exists = prev.some(m => 
        ...
        (newMessage.id.startsWith('temp-') && m.message_text === newMessage.message_text && 
         m.sender_id === newMessage.sender_id && m.dm_room_id === newMessage.dm_room_id)
      )
    ...
    setMessages(prev => {
      ...
      return prev.map(msg => 
        msg.id === tempId ? { 
          ...
          message_text: confirmedMessage.message_text || msg.message_text,
  ...
  useEffect(() => {
    ...
    const setupSubscription = () => {
      ...
      const channel = supabase.channel('direct_messages_' + user.id + '_' + dmRoomId)
      ...
      channel
        .on(
          'postgres_changes',
          {
            ...
            filter: `or(and(sender_id.eq.${user.id},recipient_id.eq.${dmRoomId}),and(sender_id.eq.${dmRoomId},recipient_id.eq.${user.id}))`
          },
          async (payload) => {
            if (!isMounted) return
  ...
  }, [user?.id, dmRoomId, addMessage, confirmMessage, removeMessage, pendingMessages])
