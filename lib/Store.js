...
export const uploadFile = async (file, bucket) => {
  try {
    ...
    // Validate bucket name and normalize
    const normalizedBucket = bucket.replace('_', '-')
    if (!['avatars', 'message-attachments'].includes(normalizedBucket)) {
    ...
    if (!bucketExists) {
      throw new Error(`Bucket "${normalizedBucket}" not found. Please try again.`)
    }
