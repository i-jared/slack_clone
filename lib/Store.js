...
const ensureStorageBuckets = async () => {
  try {
    ...
    const attachmentsBucketExists = buckets?.some(b => b.name === 'message-attachments')
    ...
    if (!attachmentsBucketExists) {
      ...
      const { error: attachmentsError } = await supabase
        .storage
        .createBucket('message-attachments', { public: true })

      if (attachmentsError) {
        console.error('Error creating message-attachments bucket:', attachmentsError)
      }
    }

    // Verify buckets after creation
    const { data: verifyBuckets } = await supabase.storage.listBuckets()
    console.log('Available buckets:', verifyBuckets?.map(b => b.name))
  } catch (error) {
...
export const useStore = (props) => {
  ...
  const fetchChannels = async () => {
    try {
      ...
      // Map the channels to include proper display names
      const mappedChannels = data.map(channel => ({
        ...channel,
        displayName: channel.name || channel.slug, // Use name if available, fallback to slug
        description: channel.description || `Welcome to #${channel.slug}`, // Use description if available, fallback to default
        name: channel.name || channel.displayName || channel.slug // Ensure name is always set
      }))
