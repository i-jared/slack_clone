...
export async function sendMessage({ message, channel_id, user_id }) {
  try {
    // First verify and get the full channel ID
    const { data: channels } = await supabase
      .from('channels')
      .select('id, name')
    
    const matchingChannel = channels?.find(c => c.id.startsWith(channel_id))
    if (!matchingChannel) {
...
export async function sendDirectMessage(content, recipient_id) {
  ...
  try {
    ...
    const fullMessageData = {
      ...
      content: insertedMessage.message_text
    }
...
// Define default channels with proper UUIDs
const DEFAULT_CHANNELS = [
  {
    id: '37a6d9e0-3b11-4c0e-8c77-45c01b0d7d9b',
    name: 'General',
    slug: 'general',
  ...
  {
    id: '8f7d3c1a-4b5e-4d2f-9e8f-12a3b4c5d6e7',
    name: 'Random',
    slug: 'random',
  ...
  {
    id: '2b9e8f4d-7c6a-5b3a-1e2d-9f8e7d6c5b4a',
    name: 'Announcements',
    slug: 'announcements',
...
const ensureDefaultChannels = async () => {
  try {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) {
      console.log('User not authenticated, skipping channel creation')
      return
    }
    ...
    if (!dbUser) {
      ...
      console.log('✅ User record created:', newUsers[0])
      return newUsers[0]
    }
    ...
    console.log('✅ User record exists, checking channels...')
    ...
    if (channelsToCreate.length > 0) {
      console.log('Creating channels:', channelsToCreate)
    ...
    console.error('❌ Error in ensureDefaultChannels:', error)
    throw error
  }
}
  ...
  useEffect(() => {
    const initializeStore = async () => {
      try {
        ...
        // Ensure channels exist and fetch data
  ...
  const fetchMessages = async (channelId) => {
    try {
      const { data, error } = await supabase
        ...
        .is('parent_id', null)  // Only fetch messages that are not replies
