  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        // Check if the channelId is a partial match of any full UUID
        const matchingChannel = channels?.find(c => c.id.startsWith(channelId))
        if (!matchingChannel) {
          console.error('❌ Channel not found:', channelId, 'Available channels:', channels?.map(c => c.id))
          return
        }

        // Use the full UUID for the query
        const { data, error } = await supabase
          ...
          `)
          .eq('channel_id', matchingChannel.id)
          .is('parent_id', null)
        ...
        console.log(`✅ Fetched ${data?.length || 0} messages for channel ${matchingChannel.name} (${matchingChannel.id})`)
        setMessages(data || [])
      } catch (error) {
